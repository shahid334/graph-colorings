...
the 1/8th of it */
#define MAX_NR_VERTICES      10000  //tvn: probably need to be BIGGER (todo)
...
void readDIMACSBinaryFormat(char *file, int &nVertices, int &nEdges){
  ...
  if(length >= MAX_PREAMBLE)
    { fprintf(stderr,"ERR: Preamble is too long.\n");exit(10);}
  ...
  if(feof(fp)){fprintf(stderr,"ERR: unexpected EOF, corrupted file.\n");exit(10);}
  if(ferror(fp)){fprintf(stderr,"Read error");	exit(10);}
